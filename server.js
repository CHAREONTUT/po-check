...
// ══════════════════════════════════════════════════════════════
// BULK UPLOAD  (map: PO No.|DM|Start|End|Name)
// receive: [{poNo,dm,start,end,name},...] from frontend after parsing xlsx
// ══════════════════════════════════════════════════════════════
app.post('/api/po/bulk', auth, async (req,res)=>{
  try{
    ...
    const grouped={}
    for(const r of rows){
      const key=(r['PO No.']||r['PO_No']||r['poNo']||'').trim()
      if(!key) continue
      if(!grouped[key]) grouped[key]={client:r['DM']||'',start:r['Start']||'',end:r['End']||'',names:[]}
      if(r['Name']&&r['Name'].trim()) grouped[key].names.push(r['Name'].trim())
    }
    ...
    for(const [poNum,data] of Object.entries(grouped)){
      ...
      const i=existingRows.findIndex(r=>r[0]===poNum)
      if(i===-1){
