...
app.get('/api/eas/pending/all',auth,async(req,res)=>{
  try{
    ...
    })).filter(r=>!r.easNo||r.status==='ยังวางไม่ครบ')
...
app.post('/api/eas',auth,async(req,res)=>{
  try{
    ...
    if(status==='ยังวางไม่ครบ'&&!remark) return res.status(400).json({ok:false,error:'กรุณากรอกหมายเหตุเมื่อยังวางไม่ครบ'})
    ...
    const finalStatus=easNo?'วางครบแล้ว':(status||'ยังวางไม่ครบ')
    const now=new Date().toLocaleString('th-TH')
...
app.put('/api/eas/:rowIdx',auth,async(req,res)=>{
  try{
    ...
    if(status==='ยังวางไม่ครบ'&&!remark) return res.status(400).json({ok:false,error:'กรุณากรอกหมายเหตุเมื่อยังวางไม่ครบ'})
    const finalStatus=easNo?'วางครบแล้ว':(status||row[4])
    const now=new Date().toLocaleString('th-TH')
    await updateRow('EAS',idx+1,[row[0],row[1],row[2],easNo||row[3],finalStatus,remark||row[5],req.user.name,now])
