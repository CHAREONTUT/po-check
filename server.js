...
// ══════════════════════════════════════════════════════════════
// PO_MASTER  cols: PONum | Client | StartPO | EndPO | CreatedBy | CreatedAt
// ══════════════════════════════════════════════════════════════
...
app.get('/api/po/:poNum',auth,async(req,res)=>{
  try{
    ...
    const row=rows[i]
    // Get employees from PO_EMPLOYEES
    const empRows=await readSheet('PO_EMPLOYEES!A2:F')
    const emps=empRows
      .map((r,idx)=>({idx:idx+2,poNum:r[0],name:r[1],start:r[2],end:r[3],createdBy:r[4],createdAt:r[5]}))
      .filter(e=>e.poNum===req.params.poNum)
...
app.post('/api/po',auth,async(req,res)=>{
  try{
    ...
    await appendRow('PO_MASTER',[poNum,client,startDate||'',endDate||'',req.user.name,now])
...
app.put('/api/po/:poNum',auth,async(req,res)=>{
  try{
    ...
    const orig=rows[i]
    const now=new Date().toLocaleString('th-TH')
    await updateRow('PO_MASTER',i+2,[req.params.poNum,client||orig[1],startDate||orig[2],endDate||orig[3],req.user.name,now])
...
// ══════════════════════════════════════════════════════════════
// PO_EMPLOYEES  cols: PONum | Name | Start | End | CreatedBy | CreatedAt
// ══════════════════════════════════════════════════════════════
...
app.post('/api/employees',auth,async(req,res)=>{
  try{
    ...
    const now=new Date().toLocaleString('th-TH')
    await appendRow('PO_EMPLOYEES',[poNum,name,start||'',end||'',req.user.name,now])
...
app.put('/api/employees/:rowIdx',auth,async(req,res)=>{
  try{
    ...
    const now=new Date().toLocaleString('th-TH')
    await updateRow('PO_EMPLOYEES',idx+1,[row[0],name||row[1],start||row[2],end||row[3],req.user.name,now])
    await log(req.user,'UPDATE_EMPLOYEE',`แก้ไขพนักงาน ${name} ใน PO ${row[0]}`)
    res.json({ok:true})
  }catch(e){ res.status(500).json({ok:false,error:e.message}) }
})
...
app.delete('/api/employees/:rowIdx',auth,async(req,res)=>{
  try{
    ...
    const row=rows[idx-1]
    if(!row) return res.status(404).json({ok:false,error:'ไม่พบรายการ'})
    await deleteRow('PO_EMPLOYEES',idx+1)
...
// ══════════════════════════════════════════════════════════════
// BULK UPLOAD  headers: PO No. | DM | Start | End | Item | Name
// ══════════════════════════════════════════════════════════════
app.post('/api/po/bulk',auth,async(req,res)=>{
  try{
    ...
    // Group unique POs first
    const poMap={} // poNum -> {client,start,end}
    let lastPO='',lastDM='',lastPOStart='',lastPOEnd=''
    const empList=[] // [{poNum,name,start,end}]
    ...
    for(const r of rows){
      ...
      if(rawPO){
        ...
        lastPOStart=str(r['Start']??r['start']??'')
        lastPOEnd=str(r['End']??r['end']??'')
      }
      ...
      const empStart=str(r['Start']??r['start']??'')
      const empEnd=str(r['End']??r['end']??'')
      if(name) empList.push({poNum:po,name,start:empStart,end:empEnd})
    }
    ...
    for(const[poNum,data] of Object.entries(poMap)){
      ...
      if(i===-1){
        await appendRow('PO_MASTER',[poNum,data.client,data.start,data.end,req.user.name,now])
        poCreated++
      } else {
        await updateRow('PO_MASTER',i+2,[poNum,data.client||existingPO[i][1],data.start||existingPO[i][2],data.end||existingPO[i][3],req.user.name,now])
        poUpdated++
      }
    }

    // Add employees (check for duplicate name+start+end in same PO)
    for(const emp of empList){
      const dup=existingEmp.find(r=>r[0]===emp.poNum&&r[1]===emp.name&&r[2]===emp.start&&r[3]===emp.end)
      if(!dup){
        await appendRow('PO_EMPLOYEES',[emp.poNum,emp.name,emp.start,emp.end,req.user.name,now])
        empAdded++
      }
    }
...
app.patch('/api/ot/:rowIdx/poot',auth,async(req,res)=>{
  try{
    ...
    await log(req.user,'UPDATE_OT_POOT',`แก้ไข PO_OT: ${req.body.poOt}`)
    res.json({ok:true})
  }catch(e){ res.status(500).json({ok:false,error:e.message}) }
})
...
app.patch('/api/ot/:rowIdx/collected',auth,async(req,res)=>{
  try{
    ...
    await updateRow('OT',idx+1,updated.slice(0,10))
    await log(req.user,'UPDATE_OT_STATUS',`${newStatus}: ${row[1]}`)
...
// ══════════════════════════════════════════════════════════════
// EAS  cols: PONum | EmpName | Month | EASNo | Status | Remark | CreatedBy | CreatedAt
// ══════════════════════════════════════════════════════════════
app.get('/api/eas',auth,async(req,res)=>{
  try{ res.json({ok:true,data:await readSheet('EAS!A2:H')}) }
  catch(e){ res.status(500).json({ok:false,error:e.message}) }
})

// GET pending EAS (ยังไม่วาง) - all POs — ต้องอยู่ก่อน :poNum
app.get('/api/eas/pending/all',auth,async(req,res)=>{
  try{
    const rows=await readSheet('EAS!A2:H')
    const data=rows.map((r,i)=>({
      idx:i+2,poNum:r[0],empName:r[1],month:r[2],easNo:r[3],
      status:r[4],remark:r[5],createdBy:r[6],createdAt:r[7]
    })).filter(r=>!r.easNo||r.status==='Not Placed')
...
app.post('/api/eas',auth,async(req,res)=>{
  try{
    const{poNum,empName,month,easNo,status,remark}=req.body
    if(!poNum||!empName||!month) return res.status(400).json({ok:false,error:'กรุณากรอก PO ชื่อพนักงาน และเดือน'})
    // Business logic: ถ้ายังไม่ครบต้องมีหมายเหตุ
    if(status==='Not Placed'&&!remark) return res.status(400).json({ok:false,error:'กรุณากรอกหมายเหตุเมื่อยังวางไม่ครบ'})
    // Check duplicate: 1 คน x 1 เดือน x 1 PO
    const rows=await readSheet('EAS!A2:H')
    ...
    // Auto status: ถ้ามีเลข EAS = วางครบแล้ว
    const finalStatus=easNo?'Placed':(status||'Not Placed')
    const now=new Date().toLocaleString('th-TH')
    await appendRow('EAS',[poNum,empName,month,easNo||'',finalStatus,remark||'',req.user.name,now])
...
app.put('/api/eas/:rowIdx',auth,async(req,res)=>{
  try{
    const sheetRow=parseInt(req.params.rowIdx)  // actual sheet row number
    const rows=await readSheet('EAS!A2:H')
    const row=rows[sheetRow-2]  // convert: row2=index0
    if(!row) return res.status(404).json({ok:false,error:'ไม่พบรายการ (row '+sheetRow+')'})
    const{easNo,status,remark}=req.body
    ...
    const now=new Date().toLocaleString('th-TH')
    await updateRow('EAS',sheetRow,[row[0],row[1],row[2],easNo||row[3],finalStatus,remark||row[5],req.user.name,now])
...
app.delete('/api/eas/:rowIdx',auth,async(req,res)=>{
  try{
    // rowIdx is the actual sheet row number (idx=i+2 from GET, where i is 0-based array index)
    // Sheet row 2 = first data row, sheet row 3 = second data row, etc.
    const sheetRow=parseInt(req.params.rowIdx)  // actual sheet row number
    const rows=await readSheet('EAS!A2:H')
    const row=rows[sheetRow-2]  // convert sheet row to array index: sheetRow-2 (row2=index0)
    if(!row) return res.status(404).json({ok:false,error:'ไม่พบรายการ (row '+sheetRow+')'})
